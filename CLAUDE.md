...
- **After changing any price sheet (`docs/app/data.jsx`, `laser-data.jsx`, `mop-data.jsx`, `icecut-data.jsx`), regenerate the Admin price mirror** so the Admin > Pricing editor and its ⬇ Excel export show the new numbers (they read this file, not the DB):
  ```bash
  node scripts/gen-price-snapshot.cjs   # writes docs/price-snapshot.json
  ```
  The generator loads those files and calls the shop's own `*Sizes`/`*Sku`/rate functions, so the mirror is exact — never re-typed. Commit `docs/price-snapshot.json` alongside the sheet change. Standard per-piece/packet categories are driven by `DESCS`; the special-unit ones (laser net, mop, ice cut, moissanite ₹/ct, navratna ₹/set, multi-sapphire ₹/ct-or-strip, lab grown) have their own blocks lower in the script. **~7,400 sheet rows + ~1,300 base-priced rows are mirrored, all cross-checked to 0 mismatches** — keep it that way (the generator prints per-category counts).
  - `__catalog__` in the snapshot carries the shop's **exact Category → Grade → Colour flow** (from `GRADES_BY_CATEGORY` + the `*_COLORS_BY_GRADE` maps); the Admin Pricing screen is driven from this, NOT the backend catalog, so it matches the storefront (grade bifurcation, per-grade colours). Keep this in sync when grades/colours change.
  - **Base-priced colours** (offered by a grade but with no sheet, e.g. Corundum Blue 34, all Beads) get generated rows via the shop's own `makeBrowseProduct`/`unitRate` helpers, flagged `base:true` and shown as "base rate (no sheet yet)" in the editor. Only categories in `BASE_OK` (standard size pad) get this; special-mode ones (opaque carat-lot, pearls string/lot, polki series) and grid-less Bracelet are left out on purpose.
- **Admin Pricing editor unit handling** (`admin-web/src/screens/Pricing.tsx`): carat/strip/set categories are listed in `CARAT_STRIP` / `FLAT_SET_LABEL` and unit labels come from `rateUnitLabel`/`rateCsvSuffix` (some vary by grade — e.g. multi-sapphire aaa = ₹/ct, others = ₹/strip; lab grown created = ₹/ct, else ₹/pc). The storefront reads overrides in `screen-browse.jsx` (`moissPerCt`, `navUnit`, `msRateFor`) and the bespoke pads (`laser-`, `mop-`, `icecut-data.jsx`).
