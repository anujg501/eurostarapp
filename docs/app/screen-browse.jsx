...
function BrowseScreen({ route, setRoute, addToCart, wishlist, toggleWishlist, persona }) {
  ...
  route.cat === 'labgrown' && grade && grade.id === 'labcorundum' && window.LABCORUNDUM_SHAPES ?
      ...
      <div className="browse-step">
          ...
          <div className="shape-pick-grid">
            ...
            const sheetSizesForCard =
              ...
              category.id === 'labwhitecorundum' && window.lwcSizes ? window.lwcSizes(s) :
...
function SizeOrderPad({ product, grade, color, shape, category, qtyBySize, setQtyBySize,
  ...
  // Lab Grown per-piece price (Beryl: Excel/₹ct×wt; Corundum: flat ₹150/ct×wt).
  const lgPiecePrice = (size) => lgCorundum
    ? (window.lgCorPrice ? window.lgCorPrice(shape, size) : null)
    : (lgBeryl && window.lgPricePc) ? window.lgPricePc(color.id, shape, size) : null;
  ...
  const lwcSheet = category.id === 'labwhitecorundum' && window.lwcSizes && window.lwcSizes(shape).length ? window.lwcSizes(shape) : null;
  ...
  const skuFor = (size) => {
    ...
    // Lab Alexandrite single-grade price sheet.
    if (category.id === 'labwhitecorundum' && window.lwcSku) {
      const a = window.lwcSku(shape, size);
      if (a) return a;
    }
    ...
    <div className="browse-step">
      ...
      <div className={`size-pad ${showWt || showPieceWt ? 'has-wt' : ''} ${navMode ? 'unit-nav' : 'unit-' + unit}`} style={{ marginTop: 4 }}>
        <div className="size-pad-head" style={navMode ? { gridTemplateColumns: '140px 150px minmax(0,1fr) 130px' } : lgCreated ? { gridTemplateColumns: '140px 150px 1fr 130px' } : undefined}>
          ...
          <span style={{ textAlign: 'right' }}>{mixedMoiss ? (lgPriced ? '₹ / pc' : 'Rate') : <React.Fragment>₹ / {navMode ? 'packet' : unit === 'pkt' ? packetPriced ? 'packet' : 'pc' : unitWord}</React.Fragment>}</span>
